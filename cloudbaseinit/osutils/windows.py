...
class WindowsUtils(base.BaseOSUtils):
    ...
    def _set_static_network_config_legacy(name, address, netmask, gateway,
                                          dnsnameservers):
        if WindowsUtils._is_ipv6_address(address):
    ...
    @staticmethod
    def _fix_network_adapter_dhcp(interface_name, enable_dhcp):
        interface_id = WindowsUtils._get_network_adapter(interface_name).GUID
        with winreg.OpenKey(
                winreg.HKEY_LOCAL_MACHINE,
                "SYSTEM\\CurrentControlSet\\services\\Tcpip\\"
                "Parameters\\Interfaces\\%s" % interface_id,
    ...
    def _is_ipv6_address(address):
        return isinstance(
            ipaddress.ip_address(six.u(address)), ipaddress.IPv6Address)

    @staticmethod
    def _set_static_network_config(name, address, netmask, gateway):
        prefix_len = ipaddress.ip_network(
            u"%s/%s" % (address, netmask), strict=False).prefixlen
        ...
        # Dhcp Enabled"
        WindowsUtils._fix_network_adapter_dhcp(name, False)
    ...
    def set_static_network_config(self, name, address, netmask, gateway,
                                  dnsnameservers):
